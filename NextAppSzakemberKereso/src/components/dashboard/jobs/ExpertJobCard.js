...
import JobStateIndicator, { EXPERT_STATE_CONFIG } from './JobStateIndicator';
...
export default function ExpertJobCard({ job, onClick }) {
      ...
      <CardActionArea onClick={onClick}>
        <CardContent>
          <Stack direction="row" flexWrap="wrap" alignItems="center" gap={1} mb={1.5}>
            ...
            <JobStateIndicator status={job.status} config={EXPERT_STATE_CONFIG} sx={{ flexShrink: 0 }} />
          </Stack>
          ...
          <Stack direction={isSmallScreen ? "column" : "row"} spacing={2}>
            <Stack spacing={0.5} sx={{ minWidth: 180, flexShrink: 0 }}>
              ...
              <LabeledRow
                label="Megrendelő"
                value={`${job.initiatingUser?.lastName} ${job.initiatingUser?.firstName}`}
              />
